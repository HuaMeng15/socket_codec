 ...
 *
 * Mechanism (unchanged from WebRTC):
 *   fill_ratio = outstanding_bytes / data_window
 *     > 1.5 : encoding_rate_ratio *= 0.9    (crash hard)
 *     > 1.0 : encoding_rate_ratio *= 0.95
 *     < 0.1 : encoding_rate_ratio  = 1.0    (backlog drained -> release)
 *     else  : encoding_rate_ratio *= 1.05   (recover, capped at 1.0)
 *   target = bwe * encoding_rate_ratio, floored at min_pushback_target.
 *
 * encoding_rate_ratio_ is PERSISTENT across calls: a burst of feedback while
 * the queue is overfull multiplies it down repeatedly, which is exactly why
 * the target crashes to the floor in one RTT rather than one step.
 */
