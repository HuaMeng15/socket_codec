...
class Flag : public FlagBase {
 public:
  ...
  void ParseValue(const std::string& value) override {
    ...
    if (!(iss >> value_)) {
      throw std::invalid_argument("Invalid value for flag --" + name_ + ": " +
                                  value);
    }
  }
...
class CmdLineParser {
 public:
  ...
  void AddStringFlag(const std::string& name, const std::string& default_val,
                     const std::string& help) {
    AddFlag<std::string>(name, default_val, help);
  }

  void AddIntFlag(const std::string& name, int default_val,
                  const std::string& help) {
 ...
 private:
  ...
  template <typename T>
  void AddFlag(const std::string& name, const T& default_val,
               const std::string& help) {
